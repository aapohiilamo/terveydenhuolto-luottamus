...
plot <- ggplot(instituutiot_long, aes(x = year_decimal, y = luottamus, color = instituutio)) +
  ...
  labs(title = "Luottamuksen arvo (0-10)",
       x = "",
       y = "")

file <- here("02_output", "luottamus_instituutiot_j.svg")
...
file <- here("02_output", "luottamus_maakunnat_j.svg")
...
file <- here("02_output", "luottamus_ryhmat_j.svg")
ggsave(filename = file, plot = plot, device = "svg", width = 12, height = 6)
